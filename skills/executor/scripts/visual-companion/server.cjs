...
// RECORD_DIR is the reasoning record and may be git-tracked: screens and the
// choices the user clicked. STATE_DIR is operational state — server-info, the
// log, and the session key — and MUST live outside every Executor store,
// which safety.md declares public. STATE_DIR is never derived from
// RECORD_DIR: that is the layout that leaked the key into docs/executor/.
// Standalone defaults put both under the OS temp dir, never in a repository.
const STANDALONE_ROOT = path.join(os.tmpdir(), 'executor-visual-companion', 'standalone-' + process.pid);
const RECORD_DIR = process.env.BRAINSTORM_RECORD_DIR || path.join(STANDALONE_ROOT, 'record');
const CONTENT_DIR = path.join(RECORD_DIR, 'content');
const EVENTS_FILE = path.join(RECORD_DIR, 'events');
const STATE_DIR = process.env.BRAINSTORM_RUNTIME_DIR || path.join(STANDALONE_ROOT, 'state');
...
const BRAND_MARKUP = '<div class="brand"><span class="brand-copy">The Executor — Visual Companion</span></div>';

function renderBranding(html) {
  return html.split('<!-- BRANDING -->').join(BRAND_MARKUP);
}
...
function startServer() {
  ...
  function onListen() {
    ...
    const info = JSON.stringify({
      ...
      screen_dir: CONTENT_DIR, events_file: EVENTS_FILE,
      state_dir: STATE_DIR, idle_timeout_ms: IDLE_TIMEOUT_MS
    });
    console.log(info);
